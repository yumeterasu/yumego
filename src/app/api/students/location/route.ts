...
type ResolveResult =
  | { ok: true; address: string; lat: number; lng: number; displayName: string }
  | { ok: false; status: number; error: string };

/**
 * Resolves free-text input into coordinates, WITHOUT writing anything --
 * tried in this order:
 ...
 */
async function resolveLocationInput(rawInput: string): Promise<ResolveResult> {
  ...
  if (coords) {
    ...
    if (!placeName) {
      ...
      // confirmation name when the URL/coordinates didn't carry one -- the
      // exact coordinates are what would get saved either way.
      try {
    ...
    return { ok: true, address: resolvedAddress, lat: coords.lat, lng: coords.lng, displayName };
  }
  ...
  if (!geoRes.ok) {
    return { ok: false, status: 502, error: "Geocoding service unavailable" };
  }
  const results = await geoRes.json();
  if (!Array.isArray(results) || results.length === 0) {
    return { ok: false, status: 404, error: "住所が見つかりませんでした / Address not found" };
  }
...
// PATCH /api/students/location
//
// Removing a saved address:  { studentId, address: null }
//
// Two-step save (address input is resolved first, then confirmed
// separately, so nothing is written until the admin has seen the result):
//   1. Look up only, no write:  { studentId, address, mode: "lookup" }
//      -> { ok, address, lat, lng, displayName }
//   2. Save the confirmed result:
//      { studentId, address, lat, lng, mode: "save" }
//      -> { ok, location }
//
// mode defaults to "save" with re-resolution for backward compatibility
// if lat/lng aren't provided (resolves address itself, then writes).
export async function PATCH(req: NextRequest) {
  const body = await req.json();
  const { studentId, address, lat, lng, mode } = body ?? {};
  ...
  try {
    if (mode === "lookup") {
      const result = await resolveLocationInput(address);
      if (!result.ok) {
    ...
    // mode === "save" (or omitted): if lat/lng were already resolved by a
    // prior lookup, save them directly -- no need to re-geocode. Otherwise
    // (older caller shape) resolve and save in one step.
    let toSave: { address: string; lat: number; lng: number };
    let displayName: string | undefined;
    if (typeof lat === "number" && typeof lng === "number") {
      toSave = { address: address.trim(), lat, lng };
    } else {
      const result = await resolveLocationInput(address);
      if (!result.ok) {
        return NextResponse.json({ error: result.error }, { status: result.status });
      }
      toSave = { address: result.address, lat: result.lat, lng: result.lng };
      displayName = result.displayName;
    }

    await setStudentLocation(studentId, toSave);
    return NextResponse.json({
      ok: true,
      location: { studentId, ...toSave },
